...
class TestBulgeGraph(unittest.TestCase):
    def setUp(self):
        self.bg = cgb.BulgeGraph(os.path.join(cbc.Configuration.test_input_dir,
    ...
    def test_loop_centroid(self):
        ...
        for d in bg.defines.keys():
            if d[0] != 's' and len(bg.edges[d]) == 1:
                ...
                self.assertTrue(np.allclose(bg.coords[d][0],
                                            bg.coords[connect][sb]))
    ...
    def test_from_dotbracket(self):
        fn = os.path.join(cbc.Configuration.test_input_dir, "1gid/prepare",
                          "temp.dotplot")
        bg = cgb.BulgeGraph()
